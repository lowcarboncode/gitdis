...
#[derive(ToValue, FromValue, Clone, PartialEq, Debug)]
pub enum Operator {
    Equal,
    NotEqual,
    GreaterThan,
...
#[derive(Clone, FromValue, ToValue, PartialEq, Debug)]
...
impl ToValueBehavior for Clause {
    fn to_value(&self) -> Value {
        match self {
            Clause::ConditionGroup(condition_group) => condition_group.to_value(),
            Clause::Condition(condition) => condition.to_value(),
        }
    }
}

impl FromValueBehavior for Clause {
    type Item = Self;

    fn from_value(value: Value) -> Option<Self::Item> {
        let condition_group = ConditionGroup::from_value(value)?;
        Some(Clause::ConditionGroup(condition_group))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_condition_equal_array() {
        let clause = Clause::condition(Operator::Equal, "name", Value::from(vec!["John", "Jane"]));

        let value = Value::from(vec![("name", Value::from(vec!["John", "Jane"]))]);
        let result = match clause.execute(&value) {
            Ok(result) => result,
            Err(err) => panic!("{}", err),
        };
